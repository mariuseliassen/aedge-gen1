...
const ROYALTY_RECEIVER = "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266";
...
describe(token + " tests", function () {
  ...
  beforeEach(async function () {
    ...
    const disableWhitelist = await myToken.setWhitelistEnabled(false);
    await disableWhitelist.wait();
  ...
  it("Should transfer one", async function () {
    ...
    const addressTokens = await myToken.connect(minter).getAddressTokens(minter);
    ...
    const addressTokens2 = await myToken.connect(recipient).getAddressTokens(recipient);
  ...
  it("Bulk mint", async function () {
    ...
    let addressTokens = await myToken.connect(addr1).getAddressTokens(addr1.address);
    ...
    addressTokens = await myToken.connect(addr2).getAddressTokens(addr2.address);
    ...
    addressTokens = await myToken.connect(addr1).getAddressTokens(addr1.address);
  ...
  it("Enable whitelist", async function () {
    const enableWhitelist = await myToken.setWhitelistEnabled(true);
    await enableWhitelist.wait();

    const whitelistEnabled = await myToken.isWhitelistEnabled();
    expect(whitelistEnabled).to.equal(true);

    await expect(
      myToken.payToMint(1, { value: ethers.utils.parseEther("0.01") })
    ).to.be.revertedWith("Only whitelist can mint");
  });
  ...
  it("Disable whitelist", async function () {
    ...
    await expect(
      myToken.whitelistMint(1, merkleTree.getHexProof("invalid"), {
        value: ethers.utils.parseEther("0.01"),
      })
    ).to.be.revertedWith("Whitelist period is over");
  });
  ...
  it("Can not mint with wrong price", async function () {
    ...
    ).to.be.revertedWith("Not enough ether provided to mint!");
  });
  ...
  it("Can not mint above max per wallet", async function () {
    ...
    const maxPerWallet = await myToken.getMaxPerWallet();
